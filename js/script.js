    ...
    contactForm.addEventListener('submit', function(e) {
        ...
        .then(async (response) => {
            ...
            if (response.status === 200) {
                ...
                showNotification('Error: ' + (jsonResponse.message || 'Something went wrong!'), 'error');
            }
        })
        .catch(error => {
            console.error('Network error:', error);
            showNotification('Something went wrong! Please try again or contact me directly via email.', 'error');
        })
        .finally(() => {
            // Reset button state
...
function initAccessibility() {
    ...
    const interactiveElements = document.querySelectorAll('.project-card, .skill-tag, .info-card');
